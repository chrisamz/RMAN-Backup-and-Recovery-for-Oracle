...
BACKUP_FILE="$BACKUP_DIR/differential_backup_$TIMESTAMP"

# Perform the differential backup using RMAN
...
if [ $? -eq 0 ]; then
  echo "[$(date)] Differential backup successful: $BACKUP_FILE" >> ../logs/backup.log
else
